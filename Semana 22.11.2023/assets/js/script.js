...
let crescimentoMensal = mediaCrescimentoMensal()
...
let previsaoVendas = vendas.map((venda, index) => {
    return {
...
function inserirVenda(){
    let venda = document.querySelector('#input-venda').value
    let ultimoMes = vendas[vendas.length - 1].mes
    let ano = vendas[vendas.length - 1].ano
...
function recarregaTabelasVendas(){
    ...
    vendaAno.innerHTML = ``
    colunasTabVendas.innerHTML = ``
    linhaTabVendas.innerHTML = ``

    projAno.innerHTML = ``
    mesesProj.innerHTML = ``
    linhaOtimo.innerHTML = ``
    linhaRegular.innerHTML = ``
    linhaRuim.innerHTML = ``
